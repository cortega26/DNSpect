...
| [040 — design-system rollout](040-design-system-rollout.md) | P1 / L | 039 | **Complete** — merged `f285323` (verifier: fonts self-hosted ×4 files, font-specific grep clean, dark default; note: the plan's literal `Inter` grep now false-positives on `setInterval` — any re-check must use the font-name grep) |
| [041 — two-mode IA](041-two-mode-ia.md) | P1 / L | 040 | **Complete** — merged `46ad483` (verifier: ModeSwitcher/QuickCheckPanel/LabWorkspace present, `mode.quick|mode.lab` ×6 i18n hits) |
| [042 — typographic hierarchy](042-typographic-hierarchy.md) | P1 / S | 040, 041 | **DONE, merge pending** — branch `plan/042-typographic-hierarchy` (`3fd48c9`, commits `83c6680`, `d66fff3`); merge-tree vs HEAD `609475f` is conflict-free (043 touched styles.css/screenshots after 042's base, but no overlapping hunks); scale table absent on HEAD as expected — **merge before executing 044** (044's probe asserts type-scale sizes) |
| [043 — revamp polish](043-revamp-polish.md) | P2 / S-M | 040, 041, 042 | **Complete** — merged `704494a` (verifier: `stalenessState` in runtime.ts + LiveRankingPanel, `updatedSlow|updatedStalled` ×6 i18n hits, token vars in ChartsPanel, placeholder-pinning cases in i18n.copy.test.ts:46-78). Merged ahead of 042 — safe: 043's hunks don't overlap 042's scale hunks (merge-tree conflict-free), but 042's screenshot refresh will overwrite 043's `instrument-lab-benchmark.png`; accept 042's version at merge |
| [044 — light theme repair](044-light-theme-repair.md) | P1 / S | 040-043 | TODO — drift-checked 2026-09-15 at `609475f`: zero drift (`git diff --stat 704494a..HEAD` empty on in-scope paths); Current-state excerpts re-verified accurate (light block `styles.css:97-131` still legacy-aliases only, no new-token overrides). **Executable once 042 is merged** (its probe asserts 042's type-scale sizes) |
...
(ModeSwitcher + Quick check + Lab sub-nav), 042 typographic hierarchy (user-requested type-scale fix), 043 chart/visualization re-skin + verdict-template pinning. Planned at `b70ca05`.
