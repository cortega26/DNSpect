...
| [046 — Flatpak sources regen](046-flatpak-sources-regen.md) | P1 / S | — (sequence after 045 if in flight) | **BLOCKED** — regen complete and correct on branch `plan/046-flatpak-sources-regen` (`f5bf8cd`: 582-entry valid JSON, vitest 4.1.11, scope-clean), but `make flatpak-validate` fails deterministically: manifest `commit: 9e8d552` pins the vitest-4.1.10 tree while regenerated sources reflect the current 4.1.11 lockfile. The `commit:` bump is release-tag-time work (RELEASE_CHECKLIST §6, out of scope) — validate cannot go green until then. Reviewer-verified skew (pin's lockfile → 4.1.10; new sources → 4.1.11, zero 4.1.10 refs). Decision needed: merge regen now + defer validate to release, or hold branch until tag time. Plan nit recorded: Step 2's shape-grep doesn't match the generator's real schema (`sha512`/`contents`/`type`/`dest-filename`) — future CI check must assert known-keys-changed instead. |
